# pq-verify v2.6.1 — PQC Implementation Verification
...
![ACVP-KEM](https://img.shields.io/badge/ML--KEM%20ACVP-270%2F270-brightgreen)
...
| **Compliance** | Does it match NIST's published vectors? | ML-KEM 270/270 + ML-DSA 615/615 = 885/885 ACVP vectors |
...
- **270/270** NIST ACVP ML-KEM vectors — keyGen + encaps + decaps byte-exact, KeyCheck bool-exact
...
Open `DEMO.ipynb` in Google Colab and run all cells. ~3 minutes to 270/270.
...
| `pqverify_acvp()` | Full NIST ACVP end-to-end ML-KEM (270/270, all 12 groups) |
| `pqverify_mldsa_acvp()` | Full NIST ACVP end-to-end ML-DSA (615/615, FIPS 204) |
| `pqverify_acvp_all()` | Both ACVP suites combined (885/885) |
...
pq_verify/
  ...
  core.py                  The stack (5679 lines, 6 field-native engines)
  cli.py                   Command-line interface
tests/test_pqverify.py     16-test pytest suite
pyproject.toml             Build config + console-script entry point
dist/
  pq_verify-2.6.1-py3-none-any.whl    Installable wheel
  pq_verify-2.6.1.tar.gz              Source distribution
DEMO.ipynb                 One-click Colab demo → 270/270
...
Install: `pip install dist/pq_verify-2.6.1-py3-none-any.whl`
...
**For the full 160/160 self-suite and the 270/270 ACVP claim:**
