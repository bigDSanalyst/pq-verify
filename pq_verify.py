...
def audit_external(lib, impl_name):
    ...
    if impl_name == 'kyber-py':
        try:
            ...
            result.add_test('Import kyber-py', True, 'Package found')
            pk, sk = Kyber512.keygen()
            result.add_test('Keygen executes', True, f'pk={len(pk)}B, sk={len(sk)}B')
            ct, ss_enc = Kyber512.enc(pk)
            result.add_test('Encapsulate executes', True, f'ct={len(ct)}B')
            ss_dec = Kyber512.dec(ct, sk)
            match = ss_enc == ss_dec
            result.add_test('Shared secret match', match)
            if not match:
                result.add_finding('CRITICAL', 'Shared secrets do not match after enc/dec')
            result.add_finding('INFO', 'kyber-py functional audit complete')
        except ImportError:
...
def gen_coq_cert(n, q, poly_in, butterfly_results, zeta):
    ...
    for j in range(min(n // 2, 16)):
        ...
        lines.append(f"Proof. reflexivity. Qed.")
